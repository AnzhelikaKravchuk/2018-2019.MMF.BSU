...
Разроботать класс String, предоставляющий несколько перегруженных конструкторов, в том числе конструктор копирования. Реализовать методы для выполнения конкатенации строк (Concat), а также отношения эквивалентности (Equals) и порядка (CompareTo). Перегрузить операции сложения (+), индексирования ([]), отношения (==, !=, <, >  т.д.), присваивания для данного класса. Создать массив объектов типа String и передать его в функцию (не метод класса), которая выполняет сортировку строк.
